...
public:
	static bool Read(std::string &file_name, DoASeq *seq)
	{
		// 
		try {
			...
			std::cout << "DoASeq Read " << e.what() << '\n';
			return(false);
		}
		return(true);
	}

	static bool Write(DoASeq *seq, std::string &file_name)
	{
		try {
