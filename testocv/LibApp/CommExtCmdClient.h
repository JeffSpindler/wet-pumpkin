#ifndef COMMEXTCMDCLIENT_H
#define COMMEXTCMDCLIENT_H
...
class CommExtCmdClient
{
public:
	CommExtCmdClient(std::string name_str = default_name) {	m_name = default_name;
															m_enable_flag = false;
															m_alg = 0;
															m_DidExec = false; 
	...
	CommExtCmdClient(std::string host_str, std::string port_str) : 
											m_host_str(host_str), m_port_str(port_str) { 
															m_enable_flag = true;
															m_alg = 0;
															m_DidExec = false; };
	~CommExtCmdClient() { m_DidExec = false; };
...
BOOST_CLASS_VERSION(CommExtCmdClient, 2)
